...
const serverPubKey = Buffer.from(
  "1bfbc81ce01bc3ee5e19f8d8014f46b257d0deef59300df37c43391fde003930",
  "hex",
);
...
const main = async () => {
  ...
  await client.request(
    AuctionCommands.sub,
    Buffer.from(rpcServer.publicKey.toString("hex"), "utf-8"),
  );
  rpcServer.respond("event", (data) => {
    try {
      ...
      if (e.winnerName)
        setTimeout(() => {
          console.info(
            `CONGRATS! ${winner} WON AUCTION ${e.auction?.name} WITH BID OF ${e.winnerPrice}`,
          );
        }, 500);
    } catch (err) {
